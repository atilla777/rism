...
```
Далее необходимо в корне приложения создать файл **.env.development**, скопировать в него строки из файла **.env.production**, и указать в файле **.env.development** у четную запись пользователя Postgresl (**rism**), ее пароль и API ключ Shodan (для его получения необходимо зарегистрироваться на сайте [Shodan]( https://www.shodan.io/)).
